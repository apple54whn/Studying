...
* 元素**唯一**。与`Collection`接口的方法一致，**没有索引，只可以迭代器或foreach**
* 使用Set集合保存**自定义对象（可为 null）**，，除过`TreeSet`其他俩**必须重写**`hashCode()`和`equal()`方法
* 其实现类都重写了`toString()`方法

### 4.6.1 HashSet

- 底层数据结构是**哈希表（元素为链表或红黑树的数组）**，实际上是一个`HashMap`实例，查询快。**自动按`hashCode`排序**，但**迭代出的元素顺序和存入顺序不一致**，即**不稳定**（无序）。


### 4.6.2 LinkedHashSet

- 继承`HashSet`，底层数据结构是**链表和哈希表（数组+链表/红黑树），元素迭代顺序和存入顺序一致**

### 4.6.3 TreeSet

- 底层数据结构是**红黑树(是一个自平衡二叉树)，并且有序**，使用`TreeSet`保存自定义元素，这个元素**必须实现`Comparable`接口**或构造时**必须提供`Comparator`实现类**
