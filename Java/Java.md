...
UTF-8 最大的一个特点，就是它是一种变长的编码方式。它可以使用 1~4 个字节表示一个符号。从 unicode 到 uft-8 并不是直接的对应，而是要过一些算法和规则来转换(即 Uncidoe 字符集≠UTF-8 编码方式)。
...
![Xnip2019-07-27_12-55-25](images/Xnip2019-07-27_12-55-25.jpg)

![Xnip2019-07-27_12-55-41](images/Xnip2019-07-27_12-55-41.jpg)
...
- 变量是程序中最基本的存储单元。格式： 数据类型 变量名 = 数据值;
...
* **基本数据类型**：整数型、浮点型、字符型、布尔型
  ...
  | 整型         | short                             | 2个字节  | -2^15~2^15-1 = -32768~32767    |
  | 整型         | int（默认）                       | 4个字节  | -2^31~2^31^-1（约21亿）        |
  | 整型         | long                              | 8个字节  | -2^63~2^63^-1                  |
  | 单精度浮点数 | ﬂoat（尾数可以精确到7位有效数字） | 4个字节  | -3.403E38~3.403E38             |
  | 双精度浮点数 | double（默认）                    | 8个字节  | -1.798E308~1.798E308           |
...
* **数据类型转换**
  ...
  * **自动类型转换**（隐式），范围**小**的类型向范围**大**的**类型提升**，如下：
    ...
    **参与运算时byte、char、short**——>**int**——>**long**——>**float**——>**double**（boolean不参与）
  ...
  * **强制类型转换**（显式），是自动类型转换的逆过程，将容量大的数据类型转换为容量小的数据类型。
  
    一般不推荐使用，有可能发生**精度损失**（浮点转成整数，直接取消小数），**数据溢出**
...
* **算术运算符**：【+】【-】【*】【/】【%】【++】【--】
  ...
  * 对于除号“/”，它的整数除和小数除是有区别的:整数之间做除法时，只保留整数部分而舍弃小数部分。
    ...
    例如:int x=3510;x=x/1000*1000，最终x值为3000
...
* **位运算符**
  ...
  * `<<`左移，被移除的高位丢弃，空缺位补0。如`3 << 2 = 12 --> 3*2*2=12`

  * `>>`右移，被移位的二进制最高位是0，右移后所有空缺位补0；最高位是1，空缺位补1。如`3 >> 1 = 1 --> 3/2=1`

  * `>>>`无符号右移，被移位二进制最高位无论是0或者是1，空缺位都用0补。，如`3 >>> 1 = 1 --> 3/2=1`
...
* **跳出语句**
  ...
  * **break**：**终止switch**或者**当前循环**，用在if中没用。

    可以用label来指定指明要终止的是哪一层语句块，否则只终止一层，标号语句必须紧接在循环的头部。
...
* **数组**： 存储数据**长度固定**的容器，多个数据的**数据类型要一致（存在自动类型转换）**。是**引用数据类型**， 创建数组对象会在内存中开辟一整块连续的空间，而数组名中引用的是这块连续空间的首地址。  						 					 				 			 		
